...
public class LibraryManager implements ActionListener, PopupMenuListener, Notifiable {
  ...
  public void actionPerformed(final ActionEvent e) {
    ...
    if (!m_inUpdate) {
      if ("comboBoxChanged".equals(e.getActionCommand()) && !getCurrentLibraryName().equals(newLibraryName)) {
      ...
      new Runnable() {
        public void run() {
          synchronized (m_Lock) {
            ...
            final Preferences prefs = Preferences.userRoot().node(libraryRoot + '/' + lName);
  ...
  public void saveLibrary(String libraryName) {
    ...
    final Preferences prefs = Preferences.userRoot().node(libraryRoot + '/' + libraryName);
    ...
    EventManager.getInstance().fireEvent(this, LibraryManager.EVENT_SAVE, libraryName);
  }
...
//    System.out.println("Delete " + libraryName);
    ...
    try {
      prefs.removeNode();
      EventManager.getInstance().fireEvent(EventManager.getGlobalTarget(), LibraryManager.EVENT_DELETE, libraryName);
    } catch (BackingStoreException e) {
...
//    System.out.println( "lib" + libraryName + ":" + getCurrentLibraryName() );
  ...
  public static void importFile(final String fileName) {
    ...
    try {
      ...
      is.close();
      EventManager.getInstance().fireEvent(EventManager.getGlobalTarget(), LibraryManager.EVENT_IMPORT, null);
    } catch (Exception e) {
      Utils.handleException("error.importingLibrary", e);
    }
  }
