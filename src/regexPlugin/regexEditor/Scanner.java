 ...
 *    try { doc.getTextComponent(offset, length, text); }
...
public class Scanner
{
    ...
    private void moveGap( final int newgap )
    {
        if ( scanning )
            throw new IllegalStateException( "moveGap called while scanning" );
        if ( newgap < 0 || newgap > gap + tokens.length - endgap )
        {
            throw new IllegalStateException( "bad argument to moveGap" );
        }
        if ( gap < newgap )
        {
            while ( gap < newgap )
    ...
    public int size()
    {
        if ( scanning ) return gap;
        else return gap + tokens.length - endgap;
    }
    ...
    {
        if ( n < 0 || n >= gap && scanning ) return null;
        if ( n >= gap ) moveGap( n + 1 );
        return tokens[n];
    }
    ...
    public int find( final int p )
    {
        ...
        if ( !scanning ) moveGap( gap + tokens.length - endgap );
        end = gap - 1;
        if ( p > tokens[end].position ) return end;
        while ( end > start + 1 )
        {
            mid = (start + end) / 2;
     ...
     */
    public int change( final int start, final int len, final int newLen )
    {
        if ( start < 0 || len < 0 || newLen < 0 || start + len > textLength )
        {
            throw new IllegalArgumentException(
                "change(" + start + "," + len + "," + newLen + "," + textLength + ")" );
            // return gap;
        }
        textLength += newLen - len;
        final int end = start + newLen;
        if ( scanning )
        {
            while ( gap > 0 && tokens[gap - 1].position > start ) gap--;
            if ( gap > 0 ) gap--;
            if ( gap > 0 )
            {
                gap--;
        ...
        if ( endgap == tokens.length ) moveGap( gap - 1 );
        scanning = true;
     ...
     */
    public int scan( final char[] array, final int offset, final int length )
    {
        if ( !scanning )
        {
            throw new IllegalStateException( "scan called when not scanning" );
        }
        if ( position + length > textLength )
        {
            throw new IllegalArgumentException( "scan too much" );
        }
        final boolean all = position + length == textLength;
        end = start + length;
        final int startGap = gap;
        ...
        while ( start < end )
        {
            ...
            if ( type != TokenType.WHITESPACE )
            {
                ...
                final Token t = new Token( sym, position );
                if ( gap >= endgap ) checkCapacity( gap + tokens.length - endgap + 1 );
                tokens[gap++] = t;
            }

            // Try to synchronise

            while ( tokens[endgap].position + textLength < position ) endgap++;
            if ( position + start - tokenStart == textLength ) scanning = false;
    ...
    private void checkCapacity( final int capacity )
    {
        ...
        if ( capacity <= oldCapacity && 4 * capacity >= oldCapacity ) return;
        final Token[] oldTokens = tokens;
        int newCapacity;
        ...
        final int n = oldCapacity - endgap;
    ...
    void print()
    {
        ...
        {
            if ( i >= gap && i < endgap ) continue;
            if ( i == endgap ) System.out.print( "... " );
            System.out.print( "" + i + ":" + tokens[i].position );
            System.out
                .print( "-" + (tokens[i].position + tokens[i].symbol.name.length()) );
            System.out.print( " " );
        }
        System.out.println();
    }
