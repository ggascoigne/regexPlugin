...
public class ParenHighlighter implements CaretListener {
  ...
  public ParenHighlighter() {
    this(new DefaultHighlighter.DefaultHighlightPainter(new Color(153, 204, 255)),
   ...
   */
  public static char getCharAt(final Document doc, final int p)
   ...
   */
  public static int findPrevMatchingParen(final Document doc, final int paren)
    throws BadLocationException {
    int parenCount = 1;
    int i = paren - 1;
    for (; i >= 0; i--) {
   ...
   * @return the position of the matching paren, or -1 if none is found
   */
  public static int findNextMatchingParen(final Document doc, final int paren)
    throws BadLocationException {
    int parenCount = 1;
    int i = paren + 1;
  ...
  public void caretUpdate(final CaretEvent e) {
    ...
    try {
      ...
      if (highlight) {
        if (pointB >= 0) {
          if (c2 == '(' && c == ')' || c2 == '{' && c == '}' || c2 == '[' && c == ']') {
            m_start = m_highlighter.addHighlight(pointB, pointB + 1, m_goodPainter);
            m_end = m_highlighter.addHighlight(pointA, pointA + 1, m_goodPainter);
          } else {
            m_start = m_highlighter.addHighlight(pointB, pointB + 1, m_badPainter);
            m_end = m_highlighter.addHighlight(pointA, pointA + 1, m_badPainter);
          }
        } else {
          m_end = m_highlighter.addHighlight(pointA, pointA + 1, m_badPainter);
        }
