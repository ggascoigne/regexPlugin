...
public class Regex2JavaString {
  ...
  public static String quote(final String s, int flags) {
    ...
    for (Iterator iterator = list.iterator(); iterator.hasNext();) {
      final String s1 = (String) iterator.next();
      if (commentFlag) {
        sb.append('"').append(s1).append(spaces.substring(0, longestLength - s1.length() + 2));
        if (!iterator.hasNext()) {
          sb.append("\\n\"\n");
        } else {
          sb.append("\\n\" +\n");
   ...
   */
  static public Collection splitToJavaLines(final String in, final boolean trimTrailingSpaces) {
